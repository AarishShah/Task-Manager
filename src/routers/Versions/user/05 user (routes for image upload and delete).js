...
{
    req.user.avatar = req.file.buffer // this won't work with "dest: 'avatars'" as enabled
    await req.user.save()
    res.send()
}, (error, req, res, next) =>
{
...
router.get('/users/:id/avatar', async (req, res) =>
{
    try
    {
        ...
        res.set('Content-Type', 'image/jpg') // a key value pair is taken by 'set'; first one is name of the response header and sencond is the value that we are trying to set on it (which by default is set to application/json)
