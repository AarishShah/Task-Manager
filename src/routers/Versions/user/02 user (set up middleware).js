...
router.patch('/users/:id', async (req, res) =>
{
    ...
    try
    {
        ...
        updates.forEach((update) => // updates is array of strings, update is a string
        {
            user[update] = req.body[update] // user.name = 'The_name', but user may want to change something else therefore we will use bracket notation
        }
        )

        */

        // We will use a shorthand notation of the above commented code
        updates.forEach((update) => user[update] = req.body[update])

        await user.save() // here our middleware will be executed

        // the below lines bypasses middleware, to tweak it we will ues a traditional method
